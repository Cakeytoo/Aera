  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    db.get("SELECT name, prePrompt FROM users WHERE id = ?", [decoded.id], (err2, user) => {
      ...
      const finalConversationHistory = conversationHistory || "";
        ...
        console.log("AI Script Path:", pythonScript);
        console.log("Model Path:", modelPath);
        
        // Check if files exist before proceeding
        if (!fs.existsSync(pythonScript)) {
          console.error("Python script not found:", pythonScript);
          return res.json({ success: false, error: "AI service not configured" });
        }
        if (!fs.existsSync(modelPath)) {
          console.error("Model file not found:", modelPath);
          return res.json({ success: false, error: "AI model not found" });
        }
        ...
        const tryNextPython = () => {
          if (commandIndex >= pythonCommands.length) {
            ...
            return res.json({ success: false, error: "Python not available" });
          }
          ...
          try {
            pythonProcess = spawn(pythonCmd, [pythonScript, "--service"], {
            ...
            const timeout = setTimeout(() => {
              if (!responseReceived) {
                pythonProcess.kill();
                res.json({ success: false, error: "AI response timeout" });
              }
              ...
              if (code === 0 && aiResponse.trim()) {
                try {
                  ...
                  if (result.response) {
                    ...
                    res.json({ success: false, error: result.error || "AI service error" });
                  }
                } catch (e) {
                ...
                console.log("AI process failed, trying next Python command");
                commandIndex++;
                tryNextPython();
              }
            });
            ...
            pythonProcess.on("error", (error) => {
              clearTimeout(timeout);
              if (responseReceived) return;
              console.error(`Failed to start Python process with ${pythonCmd}:`, error.message);
              commandIndex++;
              tryNextPython();
            });
